...
--4. Um dos jogos infantis mais populares no planeta, chamado de Jogo Da Forca, consiste na escolha de letras do alfabeto latino para tentar encontrar uma palavra que esteja oculta. Você deve implementar uma versão deste jogo em Haskell considerando as seguintes restrições: você deverá usar caracteres ASCII para representar a forca, o enforcado, e os espaços selecionados para cada palavra; os símbolos usados nas palavras serão apenas os caracteres do alfabeto latino minúsculos entre a e z; o banco de dados de palavras deve conter, no mínimo 10 palavras de 6 letras ou mais; a cada tentativa a tela precisa ser limpa e redesenhada (ANSI Escape Codes · GitHub). No repl.it não é possível importar a biblioteca responsável pela geração de números randômicos, pode gerar as palavras e usá-las em sequência ou, se preferir, use algum fator do sistema, como os últimos dígitos da data corrente (Dates and Times (sourceforge.net))
