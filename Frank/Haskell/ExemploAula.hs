...
-- init, last
  ...
  -- length 
  putStrLn("\nlength 1: " ++ show ( length([[1,2,4,5],[3,4,7,6], [8,9,1,3]])))
  putStrLn("\nlength 2: " ++ show ( length( head([[1,2,4,5],[3,4,7,6], [8,9,1,3]]))))
...
-- replicate - cria uma lista com n elementos iguais
 ...
 --dropWhile: retira o maior segmento inicial da lista onde uma condição é verdadeira
  ...
  putStrLn("\nzip " ++ show (zip ['a','b','c'] [2,4,5,3,8,6,7]))
